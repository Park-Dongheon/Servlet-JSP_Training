...
var webSocket = new WebSocket(
		"ws://192.168.0.128:8080/MustHaveJSP/ChatingServer")
...
function sendMessage() {
	...
	webSocket.send(chatId + "|" + chatMessage.value);	// 서버로 전송, 조립 결과 ex) => "must|/have 안녕하세요?"
...
webSocket.onmessage = function(event) {
	...
	var sender = message[0];					// 보낸 사람의 대화명, must
	var content = message[1];					// 메시지 내용, "/have 안녕하세요."
	if (content != "") {						// 메시지가 빈 값이 아닌지 확인
		if (content.match("/")) {				// 귓속말, 귓속말인지(/ 포함 여부) 확인
			if (content.match("/" + chatId)) {	// 나에게 보낸 메시지만 출력, 자신에게 보낸 것인지 확인
				// 귓속말 명령어 부분을 '[귓속말] : ' 문자열로 대체 후 채팅창에 출력
				var temp = content.replace(("/" + chatId), "[귓속말] : ");		
				chatWindow.innerHTML += "<div>" + sender + "" + temp + "</div>";
			}
		}
		else {	// 일반 대화, 일반 메시지일 때의 출력
